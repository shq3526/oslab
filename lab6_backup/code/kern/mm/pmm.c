               ...
               bool share)
{
    ...
    do
    {
        ...
        if (*ptep & PTE_V)
        {
            ...
            // [修改说明]: 原Lab6代码在此处直接调用 alloc_page()。
            // 为了支持 fork 的 COW (写时复制) 机制，我们不能无条件分配新页，
            // 否则在 share=1 时会导致内存泄漏。将初始化改为 NULL。
            struct Page *npage = NULL; 
            
            assert(page != NULL);
            // assert(npage != NULL); // [修改说明]: 移到下方分配成功后检查
            int ret = 0;
            ...
            // ==========================================================
            // Logic form Lab 5 (Adapted for Lab 6)
            // ==========================================================
            
            if (share) {
                // --- Case 1: Copy-on-Write (COW) 逻辑 ---
                // 如果启用了共享 (share = true)，我们只修改权限，不分配新页
                ...
                // 将该物理页映射给子进程 (to)，权限与父进程一致（只读）
                ret = page_insert(to, page, start, perm);
            }
            else {
                // --- Case 2: Deep Copy (深拷贝) 逻辑 ---
                // 对应 Lab 6 注释中描述的 "replicate content..."
                
                npage = alloc_page();
                if (npage == NULL) return -E_NO_MEM;
                assert(npage != NULL); // 恢复原 Lab6 的断言
