...
// stvec：存储 S 模式 Trap 处理程序的入口地址。实验中，它被设置为 __alltraps 的地址。

// sepc：硬件自动保存发生 Trap 时被中断指令的地址 (PC)。sret 返回时会跳转到这里。
// 它自动保存了被中断指令的虚拟地址。这份记录回答了“事发时程序执行到了哪里？”的问题，是未来恢复执行的关键。

// scause：硬件自动写入 Trap 的原因代码（例如 IRQ_S_TIMER 或 CAUSE_ILLEGAL_INSTRUCTION）。

// stval：硬件自动写入 Trap 的附加信息（例如，非法指令异常时，存的是指令码；缺页异常时，存的是目标地址）。

// sstatus：状态寄存器，包含三个关键位：

// SIE (Supervisor Interrupt Enable)：S 模式中断总开关。

// SPIE (Supervisor Previous IE)：保存 Trap 发生前的 SIE 值，供 sret 恢复。

// SPP (Supervisor Previous Privilege)：保存 Trap 发生前的特权级（0=U, 1=S），供 sret 判断应返回到哪个模式。
...
// U模式触发中断：这是最常见的情况。当用户程序执行ecall指令发起系统调用，或执行指令时发生页错误，该异常会被medeleg委托。
// 硬件将直接陷入到S模式，而不再经过M模式。这是操作系统为用户提供服务的主要通道。

// S模式触发中断：内核自身在运行时也可能遇到两种性质不同的陷阱。
// 第一种是被动陷入，例如设备I/O完成产生的外部中断，或内核代码触发的缺页异常（内核本身一般不引发缺页异常，但是在尝试访问用户空间传入的数据的时候，也可能触发）。
// 如果该中断类型已被委托，则处理流程为S模式陷入到S模式——这被称为“自陷”。
// 第二种是主动陷入，当内核需要访问硬件特权功能（如设置定时器）时，它会主动执行ecall指令。
// 此时，由于ecall在S模式下执行，特权级将从S模式提升至M模式，由M模式的固件（如OpenSBI）提供服务，处理完毕后再通过mret指令返回S模式。

// M模式触发中断：某些与最底层硬件管理紧密相关的中断，例如M模式的定时器中断或某些安全性事件，
// 通常不会被委托。它们始终在M模式内处理，由固件负责，这与操作系统的常规运行无关。