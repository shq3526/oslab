...
obj/__user_cow_stress.out:     file format elf64-littleriscv
...
int main(void) {
  ...
  800504:	e406                	sd	ra,8(sp)
  800506:	00001417          	auipc	s0,0x1
  80050a:	afa40413          	addi	s0,s0,-1286 # 801000 <shared_arr>
    cprintf("COW Stress Test: Start\n");
  80050e:	b33ff0ef          	jal	ra,800040 <cprintf>

    // 初始化
    for(int i=0; i<1000; i++) shared_arr[i] = i;
  800512:	8722                	mv	a4,s0
  800514:	4781                	li	a5,0
  800516:	3e800693          	li	a3,1000
  80051a:	c31c                	sw	a5,0(a4)
  80051c:	2785                	addiw	a5,a5,1
  80051e:	0711                	addi	a4,a4,4
  800520:	fed79de3          	bne	a5,a3,80051a <main+0x22>

    int pid1 = fork();
  800524:	bbbff0ef          	jal	ra,8000de <fork>

    if (pid1 == 0) {
  800528:	e115                	bnez	a0,80054c <main+0x54>
        // --- 子进程 1 ---
        // 此时 ref = 2 (父 + 子1)
        
        int pid2 = fork();
  80052a:	bb5ff0ef          	jal	ra,8000de <fork>
        
        if (pid2 == 0) {
  80052e:	e139                	bnez	a0,800574 <main+0x7c>
            // --- 孙子进程 ---
            // 此时 ref = 3 (父 + 子1 + 孙)
            ...
            shared_arr[500] = 9999; 
  800530:	6589                	lui	a1,0x2
  800532:	70f58593          	addi	a1,a1,1807 # 270f <_start-0x7fd911>
            cprintf("Grandchild: arr[500] = %d\n", shared_arr[500]);
  800536:	00000517          	auipc	a0,0x0
  80053a:	3d250513          	addi	a0,a0,978 # 800908 <error_string+0xe0>
            shared_arr[500] = 9999; 
  80053e:	7cb42823          	sw	a1,2000(s0)
            cprintf("Grandchild: arr[500] = %d\n", shared_arr[500]);
  800542:	affff0ef          	jal	ra,800040 <cprintf>
            exit(0);
  800546:	4501                	li	a0,0
  800548:	b81ff0ef          	jal	ra,8000c8 <exit>
        // 子进程修改
        shared_arr[500] = 8888;
        exit(0);
    }
    ...
    waitpid(pid1, NULL);
  80054c:	4581                	li	a1,0
  80054e:	b93ff0ef          	jal	ra,8000e0 <waitpid>
    
    // 父进程检查
    if (shared_arr[500] == 500) {
  800552:	7d042703          	lw	a4,2000(s0)
  800556:	1f400793          	li	a5,500
  80055a:	04f70463          	beq	a4,a5,8005a2 <main+0xaa>
        cprintf("COW Stress Test: PASSED\n");
    } else {
        cprintf("COW Stress Test: FAILED (Parent data corrupted)\n");
  80055e:	00000517          	auipc	a0,0x0
  800562:	44a50513          	addi	a0,a0,1098 # 8009a8 <error_string+0x180>
  800566:	adbff0ef          	jal	ra,800040 <cprintf>
    }

    return 0;
  80056a:	60a2                	ld	ra,8(sp)
  80056c:	6402                	ld	s0,0(sp)
  80056e:	4501                	li	a0,0
  ...
  800576:	b6bff0ef          	jal	ra,8000e0 <waitpid>
        if (shared_arr[500] == 500) {
  80057a:	7d042703          	lw	a4,2000(s0)
  80057e:	1f400793          	li	a5,500
  800582:	02f70763          	beq	a4,a5,8005b0 <main+0xb8>
            cprintf("Child: FAILED (Grandchild polluted data)\n");
  800586:	00000517          	auipc	a0,0x0
  80058a:	3d250513          	addi	a0,a0,978 # 800958 <error_string+0x130>
  80058e:	ab3ff0ef          	jal	ra,800040 <cprintf>
        shared_arr[500] = 8888;
  800592:	6789                	lui	a5,0x2
  800594:	2b878793          	addi	a5,a5,696 # 22b8 <_start-0x7fdd68>
        exit(0);
  800598:	4501                	li	a0,0
        shared_arr[500] = 8888;
  80059a:	7cf42823          	sw	a5,2000(s0)
        exit(0);
  80059e:	b2bff0ef          	jal	ra,8000c8 <exit>
        cprintf("COW Stress Test: PASSED\n");
  8005a2:	00000517          	auipc	a0,0x0
  8005a6:	3e650513          	addi	a0,a0,998 # 800988 <error_string+0x160>
  8005aa:	a97ff0ef          	jal	ra,800040 <cprintf>
  8005ae:	bf75                	j	80056a <main+0x72>
            cprintf("Child: Data intact after Grandchild exit.\n");
  8005b0:	00000517          	auipc	a0,0x0
  8005b4:	37850513          	addi	a0,a0,888 # 800928 <error_string+0x100>
  8005b8:	a89ff0ef          	jal	ra,800040 <cprintf>
  8005bc:	bfd9                	j	800592 <main+0x9a>
